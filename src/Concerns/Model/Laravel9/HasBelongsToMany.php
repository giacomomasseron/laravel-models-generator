...
trait HasBelongsToMany
{
    protected function belongsToMany(): string
    {
        ...
        foreach ($this->entity->belongsToMany as $key => $belongsToMany) {
            ...
            $content .= $this->spacer.'/**'."\n";
            $content .= $this->spacer.' * @return BelongsToMany<'.$belongsToMany->foreignClassName.'>'."\n";
            $content .= $this->spacer.' */'."\n";
            ...
            $content .= $withPivot ? str_repeat($this->spacer, 3).'->withPivot(\''.implode('\', \'', $belongsToMany->pivotAttributes).'\')'.(! $belongsToMany->timestamps ? ';' : '')."\n" : '';
